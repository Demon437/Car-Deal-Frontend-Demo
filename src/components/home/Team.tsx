...
//                     <img src={member.image} alt={member.name} className="w-full h-full object-contain transition-transform duration-700 group-hover:scale-105" />
//                   </div>
//                 </div>
...
export const Team = () => (
  <section className="py-24 bg-[#f6f7f9] relative overflow-hidden">
    {/* Glow blobs (converted to red tone) */}
    <div className="absolute inset-0 opacity-5 pointer-events-none">
      <div className="absolute top-1/2 left-1/4 w-96 h-96 bg-red-500 rounded-full blur-[120px]" />
      <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-red-400 rounded-full blur-[120px]" />
    </div>
    ...
    <div className="max-w-7xl mx-auto px-6 xl:px-0 relative z-10">
      <motion.div
        ...
        className="max-w-3xl mx-auto text-center mb-20"
      >
        <div className="inline-block px-4 py-1.5 mb-4 rounded-full bg-red-50 border border-red-200 text-red-600 text-xs font-bold uppercase tracking-widest">
          Our Leadership
        </div>
        ...
        <p className="text-gray-600 mt-4 text-lg">
          Leading the charge in Indian luxury automobile innovation.
        </p>
      </motion.div>
        ...
        {team.map((member, idx) => (
          ...
          >
            <div className="flex flex-col items-center text-center space-y-6">
              ...
              <div className="space-y-3">
                <h3 className="text-2xl font-display font-bold text-gray-900 group-hover:text-red-600 transition-colors duration-300">
                  {member.name}
                </h3>

                <div className="flex items-center justify-center gap-2">
                  <span className="h-px w-4 bg-red-300" />
                  <p className="text-red-600 font-bold uppercase tracking-[0.25em] text-[10px]">
                    {member.role}
                  </p>
                  <span className="h-px w-4 bg-red-300" />
                </div>

                <p className="text-gray-500 max-w-xs mx-auto text-sm leading-relaxed opacity-80 group-hover:opacity-100 transition-opacity duration-300">
                  {member.description}
                </p>
              </div>
            </div>
